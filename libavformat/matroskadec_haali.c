...
static void mkv_process_chapter(AVFormatContext *s, Chapter *chapter, int level, longlong offset)
{
  ...
  if (chapter->UID && chapter->Enabled && !chapter->Hidden) {
    AVChapter *avchap = avpriv_new_chapter(s, s->nb_chapters, (AVRational){1, 1000000000}, chapter->Start - offset, AV_NOPTS_VALUE, chapter->Display ? chapter->Display->String : NULL);
